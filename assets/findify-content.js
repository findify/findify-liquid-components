...
const getContentIntegrations = () => {
  ...
  Findify.state.content = contentsKeys.reduce(
    (acc, key) => ({ ...acc, [key]: { page: 1, title: contents[key].title } }),
    {}
  );
  const contentIngrationMapping = (key) => ({
    id: key,
...
const loadFindifyContent = async (contentsToLoad, pageChange = false) => {
  ...
  const loadContent = async (contentIntegrationsParam) => {
    ...
    return await Findify.utils.api.contents([
      { content_id: current.id, title: current.title, offset: getOffset() },
    ]);
  };
  ...
  const getContents = async (content) => {
    const contentsResponses = !pageChange
      ? Findify.state.contents
      : loadContent(contentIntegrations);

    const contentMapping = (response, index) => {
      // Update global object with count
      const current = contentIntegrations[index];
      if (current) {
        Findify.state.content[current.id] = {
          ...Findify.state.content[current.id],
          count: response.value.meta.total,
        };
        return {
          id: current.id,
          title: current.title,
    ...
    const mapping = (contents) =>
      contents.map(contentMapping).filter((content) => !!content);
...
>>>>>>> Stashed changes
  ...
  const getContentsLiquidQueries = async (contents) => {
    ...
    const getImageUrl = (item) =>
      item.image_url ? item.image_url : item.thumbnail_url;
    const mapItem = (item) => sanitizeItem(`${getImageUrl(item)}~${item.url}`);
    const mapQueryContents = (id, title, items, total) =>
      `${id}^${title}^${total}^${items.map(mapItem).join(";")}`;
    const transformedContents = contents.map(({ id, title, data }) => ({
      id,
      count: data.meta.total,
      data: data.items,
      query: mapQueryContents(id, title, data.items, data.meta.total),
    }));
  ...
  const getHTMLContents = async (contentQueries) => {
    ...
    for await (const response of responses) {
      ...
      const containerHTML = new DOMParser()
      ...
      const contentCards = Array.from(contentListNodes).filter(
        (node) =>
          ![
            "#text",
            "#comment",
  ...
  const contentIntegrations = contentsToLoad?.length
    ? contentsToLoad
    : getContentIntegrations();
...
const initFindifyContentEvents = () => {
  const contentIntegrations = Object.keys(
    Findify.merchantConfig.features.content
  )
  ...
  contentIntegrations.forEach(({ id }) => {
    ...
    const isContentDisplayed = (content_id) => {
      return (
        document.getElementById(`${content_id}-content`).style.display ===
        "block"
      );
    };
    ...
    const onScrollFunction = () => {
      if (
        isContentDisplayed(id) &&
        !Findify.state.content[id].loading &&
        findify.utils.isInViewport(infiniteScrollElement) &&
        hasNextPage(Findify.state.content[id])
      ) {
        Findify.state.content[id] = {
          ...Findify.state.content[id],
          page: Findify.state.content[id].page + 1,
          loading: true,
        };
        ...
        const lastPage = Math.ceil(
          Findify.state.content[id].count / contentLimit
        );
    ...
    if (
      lastPage === Findify.state.content[id].page ||
      Findify.state.content[id].count === 0
    ) {
      removeLoader(Findify.state.content[id]);
    }
