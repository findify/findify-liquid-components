(() => {
  ...
  const viewPageEvent = () => {
    ...
    const itemId = document.getElementById(productPageAnalyticsTagSelectorId)?.getAttribute('data-findify-item-id');
    const variantId = document.getElementById(productPageAnalyticsTagSelectorId)?.getAttribute('data-findify-variant-item-id');

    const sendEvent = (...props) => findify.core.analytics.sendEvent("view-page", ...props)
